...
class CamLocDataset(Dataset):
    ...
    def __init__(self,
        ...
        # Collect files recursively and map by relative path (relative to each root dir)
        def collect_files_map(base_dir: Path):
            files = {}
            for p in base_dir.rglob('*'):
                if p.is_file():
                    # use POSIX-like relative path so sorting is consistent across OS
                    rel = p.relative_to(base_dir)
                    files[rel] = p
            return files

        rgb_map = collect_files_map(rgb_dir)
        pose_map = collect_files_map(pose_dir)
        calib_map = collect_files_map(calibration_dir)

        # Optional coord files
        if self.init or self.eye:
            coord_map = collect_files_map(coord_dir)
        else:
            coord_map = None

        # Find relative keys that exist in all required maps
        # We assume the conversion kept the same relative path (possibly with different suffix for pose/calib).
        # For example, image 'subdir/frame_0001.png' -> pose 'subdir/frame_0001.txt'
        # So we match by name without extension for robustness:
        def key_without_suffix(rel: Path):
            return rel.with_suffix('')  # Path('subdir/frame_0001.png').with_suffix('') -> 'subdir/frame_0001'

        # Build mapping from base key -> file paths
        rgb_by_key = {key_without_suffix(k): v for k, v in rgb_map.items()}
        pose_by_key = {key_without_suffix(k): v for k, v in pose_map.items()}
        calib_by_key = {key_without_suffix(k): v for k, v in calib_map.items()}
        coord_by_key = {key_without_suffix(k): v for k, v in coord_map.items()} if coord_map is not None else None

        # Keys present in rgb and also in pose+calib (and coord if needed)
        required_keys = [k for k in sorted(rgb_by_key.keys()) if k in pose_by_key and k in calib_by_key]
        if coord_by_key is not None:
            required_keys = [k for k in required_keys if k in coord_by_key]

        if len(required_keys) == 0:
            raise RuntimeError('No matching rgb/pose/calibration files found. Check dataset layout.')

        # Create aligned lists (in deterministic order)
        self.rgb_files = [rgb_by_key[k] for k in required_keys]
        self.pose_files = [pose_by_key[k] for k in required_keys]
        self.calibration_files = [calib_by_key[k] for k in required_keys]
        self.coord_files = [coord_by_key[k] for k in required_keys] if coord_by_key is not None else None
