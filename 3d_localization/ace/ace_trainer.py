...
class TrainerACE:
    ...
    def train(self):
        ...
        for self.epoch in range(self.options.epochs):
            print(f"\n--- Epoch {self.epoch + 1} / {self.options.epochs} ---")
    ...
    def create_training_buffer(self):
        ...
        with torch.no_grad():
            ...
            while buffer_idx < self.options.training_buffer_size:
                ...
                for batch in tqdm(training_dataloader, desc="TR_Buff_Epoch Progress", unit="batch"):
                    (image_B1HW, image_mask_B1HW, gt_pose_B44, gt_pose_inv_B44,
                     intrinsics_B33, intrinsics_inv_B33, _, _) = batch
                    # Copy to device.
    ...
    def run_epoch(self):
        ...
        #         range(0, self.options.training_buffer_size, self.options.batch_size),
    ...
    def training_step(self, features_bC, target_px_b2, gt_inv_poses_b34, Ks_b33, invKs_b33):
        ...
        try:
            old_optimizer_step = self.optimizer.state_dict()['state'][0]['step']
        ...
        try:
            old_optimizer_step_2 = self.optimizer.state_dict()['state'][0]['step']
        except (IndexError, KeyError):
            old_optimizer_step_2 = 0
        # Only step if the optimizer stepped and if we're not over-stepping the total_steps supported by the scheduler.
        if old_optimizer_step < old_optimizer_step_2 < self.scheduler.total_steps:
            self.scheduler.step()
